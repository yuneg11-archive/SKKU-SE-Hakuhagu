...
const registration = async (event) => {
  ...
  if (school_name == "성균관대학교") {
    if (!school_mail.endsWith("skku.edu")) return responseTemplate.userRegistrationFail("성균관대학교 메일 주소가 아닙니다.");
  } else if (school_name == "경희대학교") {
    if (!school_mail.endsWith("khu.ac.kr")) return responseTemplate.userRegistrationFail("경희대학교 메일 주소가 아닙니다.");
  } else if (school_name == "아주대학교") {
    if (!school_mail.endsWith("ajou.ac.kr")) return responseTemplate.userRegistrationFail("아주대학교 메일 주소가 아닙니다.");
  }

  if (authenticator.authenticateUser(userId) == true) {
    return responseTemplate.userRegistrationFail("닉네임 또는 메일 주소가 이미 사용중입니다.");
  } else {
